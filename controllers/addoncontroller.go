...
//
// The main addon controller - uses the addon framework to deploy the RAN DU
// operators on a managed cluster if a ManagedClusterAddon CR exists in the
...
const (
	...
	// trigger the deployment of the RAN DU operator on that managed cluster
	ManagedClusterInstallRANDULabel      = "addons.open-cluster-management.io/ran-du"
	ManagedClusterInstallRANDULabelValue = "true"
...
type ranDUAgent struct {
	kubeConfig *rest.Config
}

var _ agent.AgentAddon = &ranDUAgent{}

func (h *ranDUAgent) Manifests(cluster *clusterv1.ManagedCluster,
...
func (h *ranDUAgent) GetAgentAddonOptions() agent.AgentAddonOptions {
	return agent.AgentAddonOptions{
		...
		InstallStrategy: agent.InstallByLabelStrategy(addonInstallNamespace, metav1.LabelSelector{
			MatchLabels: map[string]string{
				ManagedClusterInstallRANDULabel: ManagedClusterInstallRANDULabelValue,
			},
		}),
...
func getInstallNamespace(addon *addonapiv1alpha1.ManagedClusterAddOn) string {
	...
	// otherwise, will force the install to go into ran-du-system as the RAN DU operator is all-namespaces type
	// ran-du-system is the only supported ns it can install into (other than openshift-operators)
