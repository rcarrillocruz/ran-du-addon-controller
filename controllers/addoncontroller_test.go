...
var _ = Describe("Addoncontroller", func() {
	...
	Context("When a ManagedClusterExists", func() {
		...
		Context("When a ManagedClusterAddon for this addon is created", func() {
			...
			Context("When installing into ran-du-system namespace (the default)", func() {
				var operatorSubscription *operatorsv1alpha1.Subscription

				JustBeforeEach(func() {
					// Common checks here - When installing into a namespace for the operator (ran-du-system),
					// we should expect the manifestwork to contain:
					// - the aggregate clusterrole (needed to create an operatorgroup)
					// - the ran-du-system namespace
					// - the operator group
					...
					Expect(clusterRole.GetName()).To(Equal(
						"open-cluster-management:ran-du-product-addon-operatorgroups-aggregate-clusterrole"))
					...
					Expect(operatorGroup.GetName()).To(Equal("ran-du-product-operatorgroup"))
					...
					Expect(operatorSubscription.GetName()).To(Equal("ran-du-product"))
					Expect(operatorSubscription.GetNamespace()).To(Equal(vsNamespace))
					Expect(operatorSubscription.Spec.Package).To(Equal("ran-du-product")) // This is the "name" in json
					// More specific checks done in tests
				})

				Context("When the ManagedClusterAddOn spec uses ran-du-system as the installNamespace", func() {
					BeforeEach(func() {
				...
				Context("When the ManagedClusterAddOn spec uses an invalid ns as the installNamespace", func() {
					...
					It("Should still install to the default ran-du-system namespace", func() {
						// Code shouldn't have alterted the spec - but tests above will confirm that the
						// operatorgroup/subsription were created in ran-du-system
				...
				Context("When the annotation to override the StartingCSV is on the managedclusteraddon", func() {
					BeforeEach(func() {
						mcAddon.Annotations = map[string]string{
							controllers.AnnotationStartingCSVOverride: "randu.v1.2.3.doesnotexist",
						}
					})
					It("Should create the subscription (within the ManifestWork) with proper CatalogSourceNS", func() {
						Expect(operatorSubscription.Spec.StartingCSV).To(Equal("randu.v1.2.3.doesnotexist"))
	...
	Context("When a ManagedClusterExists with the install RAN DU addon label", func() {
		var testManagedCluster *clusterv1.ManagedCluster
		var testManagedClusterNamespace *corev1.Namespace

		BeforeEach(func() {
			// Create a managed cluster CR to use for this test - with RAN DU addon install label
			testManagedCluster = &clusterv1.ManagedCluster{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "addon-mgdcluster-",
					Labels: map[string]string{
						"vendor": "OpenShift",
						controllers.ManagedClusterInstallRANDULabel: controllers.ManagedClusterInstallRANDULabelValue,
					},
				},
			}
		...
		It("Should automatically create a ManagedClusterAddon for RAN DU in the managedcluster namespace", func() {
			vsAddon := &addonv1alpha1.ManagedClusterAddOn{}

			// The controller should create a RAN DU ManagedClusterAddOn in the ManagedCluster NS
			Eventually(func() error {
				return testK8sClient.Get(testCtx, types.NamespacedName{
					Name:      "ran-du",
					Namespace: testManagedCluster.GetName(),
